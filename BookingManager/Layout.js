...
/* ══════════════════════════════════════
   3. CONFIG — BookingManager nav only
══════════════════════════════════════ */
const LAYOUT_CONFIG = {
...
function injectStyles() {
  ...
  const css = `
    ...
    .lyt-d-logo { font-size: 1.05rem; font-weight: 800; color: #fff; letter-spacing: -0.2px; }
    .lyt-d-close {
    ...
    .lyt-d-item {
      ...
      color: rgba(255,255,255,0.5); font-size: 0.92rem; font-weight: 600;
      border: 1px solid transparent; transition: all 0.18s ease;
      background: none; font-family: 'Tajawal', inherit;
  ...
  panel.innerHTML = `
    ...
    <div class="lyt-d-user">
      ...
      <div style="min-width:0">
        ...
        <div class="lyt-d-role" id="lyt-d-role">مدير الحجوزات</div>
      </div>
    </div>
...
async function navigate(pageId) {
  ...
  const area = document.getElementById('app-main');
...
function updatePageContent(pageId) {
  ...
  _syncDrawer(pageId);
  if (typeof window.loadPage === 'function') window.loadPage(pageId);
}
