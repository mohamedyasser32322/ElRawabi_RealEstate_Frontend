  ...
  window.__pages['buildings'] = {
    ...
    init: async function () {
          ...
          <div class="bld-hdr">
            <div class="bld-hdr-l">
              ...
              <div>
                ...
                <div class="bld-hdr-s">تتبع وإدارة مراحل الإنشاء</div>
              </div>
            </div>
      ...
      async function apiReq(method, path, body) {
        ...
        // ✅ FIX: توحيد redirect لـ /unauth.html بدل /login.html
          ...
          if (done || isCurr) {
            ...
            if (isCurr) {
              ...
              imgs.forEach((img, ii) => {
                ...
                panelHtml += `<div class="bld-ph-thumb">
            ...
            if (done) {
              ...
              let rd2 = {}; if (stage?.reportData) { try { rd2 = JSON.parse(stage.reportData); } catch {} }
              const hasData = fields.some(f => rd2[f.id] !== undefined && rd2[f.id] !== null && rd2[f.id] !== '');
              ...
              if (hasData) {
                ...
                fields.filter(f => f.id !== 'notes').forEach(f => {
                  const val = rd2[f.id]; if (val === undefined || val === null || val === '') return;
                  let vh = '';
                ...
                panelHtml += `</div>`;
                if (rd2.notes) panelHtml += `<div class="bld-notes-v"><i class="ri-sticky-note-line" style="opacity:.4;margin-left:4px"></i>${esc(rd2.notes)}</div>`;
              }
              ...
              panelHtml += `<div class="bld-panel-actions">
                ...
                <button class="bld-btn bld-btn-w bld-btn-sm" data-act="rep" data-key="${def.key}" data-label="${esc(def.label)}" data-id="${stage?.id || ''}">
                  <i class="ri-file-edit-line"></i> تعديل التقرير
                </button>
              </div>`;
            }
            ...
            if (isCurr) {
              panelHtml += `<div class="bld-panel-actions">
                <button class="bld-btn bld-btn-w bld-btn-sm" data-act="rep" data-key="${def.key}" data-label="${esc(def.label)}" data-id="${stage?.id || ''}">
                  <i class="ri-file-list-3-line"></i> تقرير فني
                </button>
      ...
      function bindEv() {
        ...
        wrap.querySelectorAll('[data-bld-thumb-img]').forEach(img => {
          img.addEventListener('click', e => {
            ...
            openLb(getStageImages(stage), parseInt(img.dataset.idx, 10), img.dataset.key);
          });
        });
      }
