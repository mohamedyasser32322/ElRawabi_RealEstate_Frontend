...
(function () {
  ...
  const _css = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #filterBar{display:none;gap:8px;flex-wrap:wrap;margin-bottom:20px;align-items:center;padding:12px 16px;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:12px}
    ...
    .pill.p-rooms.active{background:rgba(78,141,245,.13);border-color:var(--accent);color:var(--accent)}
    ...
    .progress-wrap{margin:12px 0}.progress-lbl{display:flex;justify-content:space-between;font-size:.76rem;color:var(--text-muted);margin-bottom:6px}
    .progress-bar{height:5px;background:rgba(255,255,255,.06);border-radius:4px;overflow:hidden}
    .progress-fill{height:100%;background:linear-gradient(90deg,var(--accent),var(--success));border-radius:4px;transition:width .6s ease}
    ...
    .unit-box{flex:0 0 auto;min-width:115px;border-radius:11px;cursor:pointer;border:1.5px solid rgba(255,255,255,.1);transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease;position:relative;overflow:hidden}
    .unit-box::before{content:'';position:absolute;top:0;bottom:0;right:0;width:3px;border-radius:0 11px 11px 0}
    .unit-box.st-available{background:rgba(52,199,89,.07);border-color:rgba(52,199,89,.3)}.unit-box.st-available::before{background:var(--success)}
    .unit-box.st-available:hover{background:rgba(52,199,89,.14);border-color:var(--success);transform:scale(1.06);box-shadow:0 10px 24px rgba(52,199,89,.18)}
    .unit-box.st-reserved{background:rgba(255,149,0,.07);border-color:rgba(255,149,0,.3)}.unit-box.st-reserved::before{background:var(--warning)}
    .unit-box.st-reserved:hover{background:rgba(255,149,0,.13);border-color:var(--warning);transform:scale(1.06);box-shadow:0 10px 24px rgba(255,149,0,.15)}
    .unit-box.st-sold{background:rgba(255,59,48,.07);border-color:rgba(255,59,48,.3)}.unit-box.st-sold::before{background:var(--danger)}
    .unit-box.st-sold:hover{background:rgba(255,59,48,.13);border-color:var(--danger);transform:scale(1.06);box-shadow:0 10px 24px rgba(255,59,48,.17)}
    .unit-box.st-closed{background:rgba(107,122,141,.07);border-color:rgba(107,122,141,.25)}.unit-box.st-closed::before{background:var(--closed)}
    .unit-box.st-closed:hover{background:rgba(107,122,141,.13);border-color:var(--closed);transform:scale(1.04)}
    .unit-inner{padding:12px 12px 11px 16px}
    .u-status-dot{width:7px;height:7px;border-radius:50%;display:inline-block;margin-bottom:7px}
    .st-available .u-status-dot{background:var(--success);box-shadow:0 0 6px var(--success)}
    .st-reserved  .u-status-dot{background:var(--warning);box-shadow:0 0 6px var(--warning)}
    .st-sold      .u-status-dot{background:var(--danger);box-shadow:0 0 6px var(--danger)}
    .st-closed    .u-status-dot{background:var(--closed);box-shadow:0 0 4px rgba(107,122,141,.5)}
    .u-num{font-weight:800;font-size:1rem;color:var(--light);margin-bottom:4px;line-height:1}
    .u-type-badge{display:inline-block;font-size:.6rem;padding:1px 6px;border-radius:4px;font-weight:700;margin-bottom:5px}
    .u-type-roof{background:rgba(155,89,182,.2);color:#c39bd3;border:1px solid rgba(155,89,182,.3)}
    .u-type-normal{background:rgba(78,141,245,.1);color:rgba(78,141,245,.8);border:1px solid rgba(78,141,245,.2)}
    .u-divider{height:1px;background:rgba(255,255,255,.05);margin:6px 0}
    .u-info-row{display:flex;align-items:center;gap:4px;font-size:.7rem;color:var(--text-muted);margin-bottom:2px}
    .u-info-row i{font-size:.72rem}
    .u-client-row{font-size:.68rem;color:var(--text-muted);margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:95px}
    .u-status-text{font-size:.72rem;font-weight:700;margin-top:4px}
    .st-available .u-status-text{color:rgba(52,199,89,.9)}.st-reserved .u-status-text{color:rgba(255,149,0,.9)}
    .st-sold .u-status-text{color:rgba(255,59,48,.9)}.st-closed .u-status-text{color:var(--closed)}
    ...
    @media(max-width:768px){.form-row{grid-template-columns:1fr}.unit-detail-grid{grid-template-columns:1fr}.unit-box{min-width:96px}}
  `;
  ...
  window.__pages['projects'] = {
    ...
    init: async function () {
      ...
      /* ── build page skeleton into app-main ── */
      ...
      container.innerHTML = `
        ...
        <div id="filterBar">
          ...
          <div id="roomsPills" style="display:flex;gap:6px;flex-wrap:wrap;">
            <button class="pill active p-rooms" onclick="window.setRoomsFilter(0,this)">الكل</button>
          </div>
        </div>
      ...
      const TYPE_AR     = {2:'عادي',3:'روف'};

      const S = {
        view:'projects', page:1, filter:0, roomsFilter:0,
      ...
      function toType(val){
        ...
        if(typeof val==='number')return val;
        return {'groundfloor':1,'typicalfloor':2,'roof':3,'GroundFloor':1,'TypicalFloor':2,'Roof':3}[val]??2;
      }
      ...
      function setRoomsFilter(val,btn){
        S.roomsFilter=Number(val);
        document.querySelectorAll('#roomsPills .pill').forEach(b=>b.classList.remove('active','p-rooms'));
        if(btn){btn.classList.add('active','p-rooms');}
        doSearch();
      }
      function buildRoomsPills(roomSet){
        const rp=document.getElementById('roomsPills');if(!rp)return;
        rp.innerHTML=`<button class="pill ${S.roomsFilter===0?'active p-rooms':''}" onclick="window.setRoomsFilter(0,this)">الكل</button>`
          +roomSet.map(r=>`<button class="pill ${S.roomsFilter===r?'active p-rooms':''}" onclick="window.setRoomsFilter(${r},this)">${r} غرف</button>`).join('');
      }
      ...
      function doSearch(){
        ...
        else if(S.view==='units')S.filtered=S.filter===0&&S.roomsFilter===0?[...S.data]:S.data.filter(f=>getFloorUnits(f.id).length>0);
      ...
      function renderView(){
        ...
        if(S.view==='projects'){
            ...
            return `<div class="b-card" style="animation-delay:${i*50}ms" onclick="window.showUnits(${b.id},'${esc(b.name)}','${esc(b.code||'')}')">
              ...
              <div class="b-card-body">
                ...
                <div class="progress-wrap">
                  <div class="progress-lbl"><span>نسبة الإنجاز</span><span>${b.progress??0}%</span></div>
          ...
          buildRoomsPills(roomSet);
          const floorsToShow=S.filter===0&&S.roomsFilter===0?S.data:S.data.filter(f=>getFloorUnits(f.id).length>0);
          ...
          pagFloors.forEach((f,fi)=>{
            ...
            const isRoof=!!(f.isRoof||toType(f.type)===3);
            ...
            fu.forEach(u=>{
              ...
              const isUnitRoof=tp===3,rooms=Number(u.rooms)||0;
              const buyer=buyerById(u.buyerId),bName=buyer?buyerName(buyer):'';
              const typeBadgeClass=isUnitRoof?'u-type-badge u-type-roof':'u-type-badge u-type-normal';
              const typeBadgeText=isUnitRoof?'روف':'عادي';
              h+=`<div class="unit-box ${css}" onclick="window.openUnitDetail(${u.id})">
                ...
                <div class="unit-inner">
                  <div><span class="u-status-dot"></span></div>
                  <div class="u-num">${esc(String(u.unitNumber))}</div>
                  ...
                  <div class="u-status-text">${stAr}</div>
      ...
      async function showProjects(){
        S.view='projects';S.page=1;S.filter=0;S.roomsFilter=0;S.params={};
      ...
      async function showBuildings(projectId,projectName,projectCode){
        S.view='buildings';S.page=1;S.filter=0;S.roomsFilter=0;
      ...
      async function showUnits(buildingId,buildingName,buildingCode){
        S.view='units';S.page=1;S.filter=0;S.roomsFilter=0;
        ...
        try{
          ...
          if(buildingId){
            for(let i=1;i<=floorsCount;i++){
              const isRoof=(i===floorsCount),floorType=isRoof?3:2;
              ...
              if(floorId)await Promise.allSettled([
                POST('/api/Units',{unitNumber:`${i}01`,type:floorType,status:1,area:0,rooms:0,price:0,floorId:Number(floorId)}),
                POST('/api/Units',{unitNumber:`${i}02`,type:floorType,status:1,area:0,rooms:0,price:0,floorId:Number(floorId)})
              ]);
            }
          }
      ...
      async function submitAddFloor(buildingId){
        ...
        try{
          ...
          const floorId=floorRes?.id||floorRes?.floorId;
          if(floorId)await Promise.allSettled([
            POST('/api/Units',{unitNumber:`${floorNumber}01`,type:ftype,status:1,area:0,rooms:0,price:0,floorId:Number(floorId)}),
            POST('/api/Units',{unitNumber:`${floorNumber}02`,type:ftype,status:1,area:0,rooms:0,price:0,floorId:Number(floorId)})
          ]);
      ...
      function uTypeOpts(sel=2){const selNum=toType(sel);return [{v:2,l:'عادي'},{v:3,l:'روف'}].map(o=>`<option value="${o.v}" ${selNum===o.v?'selected':''}>${o.l}</option>`).join('');}
      ...
      async function submitAddUnit(){
        ...
        try{
          ...
          const newUnit=await POST('/api/Units',{unitNumber,type:utype,status:backendStatus,area,rooms,price,floorId:Number(floorId),buyerId:null});
          ...
          if(unitId&&(status===2||status===3)){
            ...
            if(status===3)await PUT(`/api/Units/${unitId}`,{unitNumber,type:utype,status:3,area,rooms,price,floorId:Number(floorId),buyerId:_selectedBuyerId});
          }
      ...
      async function editUnit(id){
        ...
        try{
          ...
          let bPaid=0,bRemain=0;
          if(u.bookingId){try{const b=await GET(`/api/Bookings/${u.bookingId}`);if(b){bPaid=b.amountPaid||0;bRemain=b.remainingAmount||0;}}catch{}}
          ...
          <div class="modal-footer">
            <button class="btn-submit" id="submitBtn" onclick="window.submitEditUnit(${id},${u.floorId},${currStatus},${u.bookingId||'null'})"><i class="ri-save-line"></i>حفظ</button>
            <button class="btn-cancel" onclick="window.closeModal()">إلغاء</button>
          </div>`,true);
      ...
      async function submitEditUnit(id,floorId,oldStatus,existingBookingId){
        ...
        setBusy('submitBtn',true);
        try{
          await PUT(`/api/Units/${id}`,{unitNumber,type:utype,status,area,rooms,price,floorId:Number(floorId),buyerId:(status===2||status===3)?_selectedBuyerId:null});
          const isNowBooked=(status===2||status===3),wasBooked=(oldStatus===2||oldStatus===3),bookingStatusId=status===3?2:1;
          if(isNowBooked&&!wasBooked)await POST('/api/Bookings',{status:bookingStatusId,amountPaid:bPaid,remainingAmount:bRemain,buyerId:_selectedBuyerId,unitId:id});
          else if(isNowBooked&&wasBooked&&existingBookingId)await PUT(`/api/Bookings/${existingBookingId}`,{status:bookingStatusId,amountPaid:bPaid,remainingAmount:bRemain,buyerId:_selectedBuyerId,unitId:id});
          else if(!isNowBooked&&wasBooked&&existingBookingId)await DELETE(`/api/Bookings/${existingBookingId}`);
      ...
      async function openUnitDetail(unitId){
        ...
        if(!u)return;
        const st=toStatus(u.status),tp=toType(u.type);
        const bc=STATUS_BADGE[st]||'sb-closed',stAr=STATUS_AR[st]||'—',typeLabel=TYPE_AR[tp]||'عادي';
      ...
      Object.assign(window, {
        ...
        openUnitDetail, handleSearch, setFilter, setRoomsFilter, goPage, buildRoomsPills,
      });
